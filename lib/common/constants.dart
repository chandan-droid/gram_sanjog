import 'package:flutter/material.dart';
import 'package:gram_sanjog/common/widgets/news_carousal.dart';
import '../model/category_model.dart';
import '../model/news_model.dart';
import '../model/top_news_model.dart';

final List<Category> categories = [
  //Category(categoryId: '0', name: 'For You', icon: Icons.recommend),
  Category(categoryId: '1', name: 'Politics', icon: Icons.gavel),
  Category(categoryId: '2', name: 'Sports', icon: Icons.sports_soccer),
  Category(categoryId: '3', name: 'Technology', icon: Icons.computer),
  Category(categoryId: '4', name: 'Business', icon: Icons.business_center),
  Category(categoryId: '5', name: 'Entertainment', icon: Icons.movie),
  Category(categoryId: '6', name: 'Health', icon: Icons.health_and_safety),
];
final List<String> languages =['Odia', 'Hindi', 'English','Telugu','Bengali'];

final List<String> contentType = ['video', 'shorts', 'post'];
